...
# GitHub Pages - jekeyll カスタマイズ方法<!-- omit from toc --> 
...
この `jekyll-theme-slate` という名前はテーマ側のリポジトリにある `_config.yml` に記述されている名前を使う。

大抵は `jekyll-theme-****` という命名になっている。
...
こちらは CSS よりやや原始的な方法になる。

テーマのリポジトリから HTML をコピーしてきて、`docs/_layouts/default.html` に置く。
...
ただしフロントマターに `lang` で `ja` のページなのか `en` のページなのか設定しておく必要がある。これは後述する言語切り替え用のナビゲーター上のラベルを切り替えるのに使う。
